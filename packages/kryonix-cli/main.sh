  ...
  case "$1" in
    ...
    *)
      ...
      if [[ "$subcommand" == "test" ]] && [[ $is_test_target -eq 1 ]]; then
        ...
        if [[ "$subcommand" == "home" ]] && [[ "$1" == "scan" || "$1" == "report" || "$1" == "duplicates" || "$1" == "plan" || "$1" == "manifest" || "$1" == "apply" || "$1" == "rollback" || "$1" == "categories" || "$1" == "explain" || "$1" == "export-memory" || "$1" == "projects" || "$1" == "diagnose" ]]; then
          extra_args+=("$1")
        else
          host_arg="$1"
        fi
...
if ! { [[ "$subcommand" == "home" ]] && [[ "${#extra_args[@]}" -gt 0 ]] && case "${extra_args[0]}" in scan|report|duplicates|plan|manifest|apply|rollback|categories|explain|export-memory|projects|diagnose) true ;; *) false ;; esac; }; then
