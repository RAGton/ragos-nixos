...
print_subcommand_help() {
  ...
  case "$sub" in
    ...
    home)
      ...
      printf '    export-memory    Exporta eventos do Home Brain para JSONL\n'
      printf '\n'
      printf '  Home Manager:\n'
  ...
  case "$1" in
    ...
    *)
      ...
      if [[ "$subcommand" == "test" ]] && [[ $is_test_target -eq 1 ]]; then
        ...
        if [[ "$subcommand" == "home" ]] && [[ "$1" == "scan" || "$1" == "report" || "$1" == "duplicates" || "$1" == "plan" || "$1" == "manifest" || "$1" == "apply" || "$1" == "rollback" || "$1" == "categories" || "$1" == "explain" || "$1" == "export-memory" ]]; then
          extra_args+=("$1")
        else
          host_arg="$1"
        fi
...
if ! { [[ "$subcommand" == "home" ]] && [[ "${#extra_args[@]}" -gt 0 ]] && case "${extra_args[0]}" in scan|report|duplicates|plan|manifest|apply|rollback|categories|explain|export-memory) true ;; *) false ;; esac; }; then
...
case "$subcommand" in
  ...
  home)
    if [[ "${#extra_args[@]}" -gt 0 ]] && [[ "${extra_args[0]}" == "scan" || "${extra_args[0]}" == "report" || "${extra_args[0]}" == "duplicates" || "${extra_args[0]}" == "plan" || "${extra_args[0]}" == "manifest" || "${extra_args[0]}" == "apply" || "${extra_args[0]}" == "rollback" || "${extra_args[0]}" == "categories" || "${extra_args[0]}" == "explain" || "${extra_args[0]}" == "export-memory" ]]; then
      needs_flake=0
    else
      needs_flake=1
    fi
  ...
  home)
    ...
    if [[ "${#extra_args[@]}" -gt 0 ]]; then
      case "${extra_args[0]}" in
        scan|report|duplicates|plan|manifest|apply|rollback|categories|explain|export-memory|help|--help|-h)
