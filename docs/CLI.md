...
- **System Ops**: Executa o backend de sistema do próprio wrapper Kryonix, preservando hooks, validações e logs do projeto.
...
- `kryonix switch`: Aplica a configuração do host atual.
  - Internamente: resolve o host e aciona o backend de ativação correspondente.
- `kryonix check`: Valida a flake e detecta erros de configuração antes de aplicar mudanças.
...
- `kryonix test`: Executa validações e testes do host selecionado.
...
Para qualquer host que não seja a máquina local, você **deve** usar a flag `--host`.

| Comando | Sintaxe Correta | Sintaxe Proibida |
| :--- | :--- | :--- |
| Check | `kryonix check --host glacier` | `kryonix check .#glacier` |
| Rebuild | `kryonix rebuild --host glacier` | `kryonix build .#glacier` |
| Test | `kryonix test --host glacier` | `kryonix test .#glacier` |
| Switch | `kryonix switch --host glacier` | `nh os switch ...` |

> [!CAUTION]
> **Sintaxe `. #host` ou `.#host`**: O wrapper `kryonix` rejeita explicitamente estas formas para evitar confusão com comandos nix puros e garantir que os hooks do projeto sejam respeitados.
> **Sudo**: Nunca use `sudo kryonix test`. O comando de teste deve rodar em espaço de usuário. O CLI solicitará sudo internamente apenas quando necessário (ex: no `switch`).
