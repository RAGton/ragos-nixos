...
- **Documentation Audit**: Executa `./scripts/doc-audit.sh` para garantir que não existam placeholders (`TODO`, `WIP`) na documentação canônica e que os comandos descritos no `USAGE.md` sejam válidos.
...
- **Falha no Job `shell`**: Erro de sintaxe em scripts ou `TODO` esquecido na documentação.
