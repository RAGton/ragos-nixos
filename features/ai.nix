    ...
    (lib.mkIf (cfg.brain.enable && cfg.brain.role == "client") {
      ...
      environment.systemPackages = [
        (pkgs.writeShellScriptBin "kryonix-search" ''
          ...
          API_KEY="''${KRYONIX_BRAIN_API_KEY:-}"
          if [ -z "$API_KEY" ] && [ -f "/etc/kryonix/brain.env" ]; then
            API_KEY=$(grep KRYONIX_BRAIN_API_KEY /etc/kryonix/brain.env | cut -d= -f2 | tr -d '"' | tr -d "'")
          fi
          ...
          if [ -z "$API_KEY" ]; then
            echo "Erro: KRYONIX_BRAIN_API_KEY não encontrada no ambiente nem em /etc/kryonix/brain.env"
            exit 1
          fi
        ...
        (pkgs.writeShellScriptBin "kryonix-stats" ''
          ...
          if [ -z "$API_KEY" ]; then
            echo "Erro: KRYONIX_BRAIN_API_KEY não encontrada."
            exit 1
          fi
